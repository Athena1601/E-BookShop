<!-- search bar code to search by author, book name, genre -->
<?php
session_start();
?>
...
<html lang="en">
  ...
  <body>
   ...
   <?php 
   $query = $_GET['search'];
   $mysqli=  mysqli_connect("localhost","root","","ebook_shop");
   $sql="UPDATE books SET b_img= LTRIM(b_img)";
   if ($result = mysqli_query($mysqli,"SELECT * FROM books
   WHERE (`b_nm` LIKE '%".$query."%') OR (`b_subcat` LIKE '%".$query."%') OR (`b_publisher` LIKE '%".$query."%')") or die(mysql_error())){
